*&---------------------------------------------------------------------*
*& Report ZABAP_JHY_06_02
*&---------------------------------------------------------------------*
*& 직접 JOIN을 통해서 데이터를 조회
*&---------------------------------------------------------------------*
REPORT zabap_jhy_06_02 MESSAGE-ID zabap_jhy_msg.
...
* 스트럭쳐-컴포넌트
* DB테이블~필드
* 클래스=>스태틱 메소드(또는 어트리뷰트)
* 참조변수->메소드(또는 어트리뷰트)



* 데이터베이스 테이블끼리 직접 JOIN 시켜서 데이터를 가져온 방식
  SELECT " 데이터를 가져올 필드
         scarr~mandt
         scarr~carrid
         scarr~carrname
         ...
         sflight~seatsmax
         sflight~seatsocc
    FROM ( scarr INNER JOIN spfli   ON scarr~carrid EQ spfli~carrid )   " JOIN 순서 = ( SCARR + SPFLI ) + SFLIGHT
                 INNER JOIN sflight ON spfli~carrid EQ sflight~carrid
                                   AND spfli~connid EQ sflight~connid

    UP TO 100 ROWS " 최대 100줄만 조회

    INTO TABLE gt_flights
   WHERE spfli~cityfrom   IN so_cityf
     AND spfli~cityto     IN so_cityt
     AND sflight~seatsocc LT sflight~seatsmax " LT 는 < 와 같은 Less Than, 보다 작다
