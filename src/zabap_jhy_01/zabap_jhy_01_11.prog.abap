*&---------------------------------------------------------------------*
*& Report ZABAP_JHY_01_11
*&---------------------------------------------------------------------*
*& Internal Table 연습 - Standard와 Sorted의 차이
*&---------------------------------------------------------------------*
REPORT ZABAP_JHY_01_11.


* Global ABAP Dictionary Type을 이용한 itab 생성 ( Header Line 없음 )
DATA GT_CARR TYPE SORTED TABLE OF SCARR WITH UNIQUE KEY CARRID.

*DATA GT_PFLI TYPE STANDARD TABLE OF SPFLI WITH NON-UNIQUE KEY CARRID CONNID.
...
DATA GT_PFLI LIKE TABLE OF GS_PFLI WITH NON-UNIQUE KEY CARRID CONNID.
...
START-OF-SELECTION.
  ...
  CASE 'X'.
    ...
    WHEN P_RA1.
      ...
      PERFORM DISPLAY_SCARR.  " CL_DEMO_OUTPUT 클래스를 이용해서 출력

    WHEN P_RA2.
      MESSAGE '라디오 버튼 2번을 눌렀을 때' TYPE 'I'.

      PERFORM SELECT_SPFLI.
      PERFORM DISPLAY_SPFLI.  " CL_SALV_TABLE 클래스를 이용해서 출력
